...
class ConfigManager:
    ...
    전담하는 클래스입니다. 이 클래스는 '어떻게' 스캔하는지,
    '무엇을' 보여주는지 알 필요가 없습니다. 오직 파일 I/O 책임만 집니다.
    """
...
class DesktopScanner:
    ...
    모든 'Windows 종속적' 로직을 전담하는 클래스입니다.
    """
    ...
    def scan_shortcuts(self) -> Dict[str, Dict]:
        """
        바탕화면을 스캔하여 아이콘 설정(config_data) 딕셔너리를 생성해 반환합니다.
        ...
        for path in desktop_paths:
            # glob을 사용해 해당 폴더의 모든 .lnk 파일을 찾음
            lnk_files = glob.glob(os.path.join(path, "*.lnk"))
            
            for lnk_file_path in lnk_files:
                file_name = os.path.basename(lnk_file_path)
                # 공용 바탕화면과 개인 바탕화면에 중복된 파일이 있을 수 있으므로 체크
                if file_name in scanned_files:
                    continue
                scanned_files.add(file_name)
                
                # 파일명을 고유 ID로 사용 (e.g., "Chrome.lnk" -> "Chrome")
                icon_id = os.path.splitext(file_name)[0]
                target_path = self.resolve_lnk(lnk_file_path)

                if not target_path:
                    print(f"정보: 스킵: '{file_name}' (대상이 없는 바로가기)")
                    continue
                ...
                # 다음 아이콘 위치를 격자 형태로 계산
                pos_x += (ICON_WIDTH + GRID_SPACING)
                if pos_x > SCREEN_WIDTH_LIMIT:
                    pos_x = GRID_SPACING
                    pos_y += (ICON_HEIGHT + GRID_SPACING)

        print(f"정보: 총 {len(new_config_data)}개의 바로가기를 스캔했습니다.")
...
class DraggableIcon(QWidget):
    ...
    def __init__(self, controller: 'AppManager', icon_id: str, 
        ...
        :param icon_id: 이 아이콘의 고유 ID (e.g., 'Chrome')
    ...
    def set_icon(self, image_path: str):
        ...
        # 이미지가 존재하지 않으면, 기본 아이콘(star.png)으로 대체
    ...
    def link_program(self):
        """(View) '프로그램 연결' 파일 대화상자를 엽니다.
        선택된 파일 경로는 컨트롤러에게 전달합니다.
        """
        file_path, _ = QFileDialog.getOpenFileName(
            self, "실행할 프로그램 선택", "", 
            "실행 파일/바로가기 (*.exe *.lnk);;모든 파일 (*.*)"
        )
        if not file_path:
            return
...
class AppManager:
    ...
    def start(self):
        ...
        if not self.config.load():
            ...
            scanned_data = self.scanner.scan_shortcuts()
        ...
        if not self.icon_windows:
            print("경고: 로드할 아이콘이 없습니다. (바탕화면에 바로가기가 없거나 설정 파일이 비어있습니다)")
    ...
    def create_windows_from_config(self):
        ...
        for icon_id, data in all_data.items():
            ...
            icon_widget = DraggableIcon(
                controller=self,  # 컨트롤러 자신을 뷰에 주입
                icon_id=icon_id,
